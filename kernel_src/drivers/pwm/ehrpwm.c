...
static void ehrpwm_channel_output_disable(struct pwm_device *p, int channel)
{
        struct ehrpwm_pwm *ehrpwm = to_ehrpwm_pwm(p);

        pm_runtime_get_sync(ehrpwm->dev);
        ehrpwm_aq_set_csfrc_load_mode(p, AQSFRC_RLDCSF_IMDT);
        ehrpwm_aq_continuous_frc(p, channel, AQCSFRC_CSF_FRCLOW);
