...
//		return cpdma_chan_submit(priv->txch, skb, skb->data,skb->len, 2, GFP_KERNEL);
//#else
		return cpdma_chan_submit(priv->txch, skb, skb->data,skb->len, 1, GFP_KERNEL);
//#endif
	}
	else
	{
...
static void cpsw_set_phy_config(struct cpsw_priv *priv, struct phy_device *phy)
{
	...
	val = miibus->read(miibus, phy_addr, MII_LPA);
    val &= (~DP83848_PAUSE_MOD);
	miibus->write(miibus, phy_addr, MII_LPA, val);
#endif
