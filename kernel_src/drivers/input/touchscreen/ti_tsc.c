...
static	int __devinit tscadc_probe(struct platform_device *pdev)
{
	...
	printk("[wds tsc probe]x.min=%d,x.max=%d,y.min=%d,y.max=%d,x.inverted=%d,y.inverted=%d\n",ts_dev->x.min,ts_dev->x.max,ts_dev->y.min,ts_dev->y.max,ts_dev->x.inverted,ts_dev->y.inverted);
