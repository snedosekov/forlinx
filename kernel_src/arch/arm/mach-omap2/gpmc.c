...
//		printk("%s ecc_type %d cs %d mode %d dev_width %d ecc_size %d\n",__FUNCTION__,ecc_type,cs,mode,dev_width,ecc_size);
		switch (mode) {
				case GPMC_ECC_READ:
...
//		gpmc_write_reg(GPMC_ECC_CONTROL,(1 << 0)|(1 << 8));//select reg 1; clr result;
		gpmc_write_reg(GPMC_ECC_CONTROL, 0x00000101);
		return 0;
}
				...
				const u_char *dat, u_char *ecc_code)
{
		...
		unsigned int val = 0;
		int i = 0;
		u_char *ecc_ptr = ecc_code;
		//printk("%s\n",__FUNCTION__);
		switch(ecc_type){
				case OMAP_ECC_BCH8_CODE_HW:
					for(i = 0;i < 4;i++){
						*(ecc_ptr++) =  gpmc_read_reg(GPMC_ECC_BCH_RESULT_3(i)) & 0xff;
